#include "stdafx.h"
#include "..\Header\Time_Pulse.h"
#include "Export_Function.h"
#include "SylphBow.h"


CTime_Pulse::CTime_Pulse(LPDIRECT3DDEVICE9 pGraphicDev)
	: CEffect(pGraphicDev)
{
}

CTime_Pulse::CTime_Pulse(const CTime_Pulse & rhs)
	: CEffect(rhs)
{
}

CTime_Pulse::~CTime_Pulse()
{
	Free();
}

HRESULT CTime_Pulse::Ready_GameObject(void)
{
	Add_Component();
...
_int CTime_Pulse::Update_GameObject(const _float& fTimeDelta)
{
...
void CTime_Pulse::LateUpdate_GameObject()
{
...
void CTime_Pulse::Render_GameObject()
{
	...
	m_pGraphicDev->SetRenderState(D3DRS_TEXTUREFACTOR, D3DCOLOR_ARGB(m_iAlpha, 255, 255, 205));
...
HRESULT CTime_Pulse::Add_Component(void)
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_ProtoComponent(L"Proto_Texture_Player_Bow_Time_Pulse_Origin"));
	 
	m_uMapComponent[ID_STATIC].insert({ L"Proto_Texture_Player_Bow_Time_Pulse_Origin", pComponent });

	return S_OK;
}

CTime_Pulse* CTime_Pulse::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CTime_Pulse* pInstance = new CTime_Pulse(pGraphicDev);
