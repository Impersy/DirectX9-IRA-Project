...
HRESULT CEffect_Leaf_Bullet_BackHit::Ready_GameObject(CGameObject* pOwner, _float fTimeDelta)
{
	...
	m_pTransformCom->m_vScale = dynamic_cast<CBullet*>(m_pOwner)->Get_Transform()->m_vScale * 1.5f;

	//CTransform* pPlayerTransCom = dynamic_cast<CTransform*>(Engine::Get_Component(L"Layer_GameLogic",))

	_vec3 vDir = *(dynamic_cast<CBullet_AxeEvilSoul_Leaf*>(m_pOwner)->Get_Direction());

	m_pTransformCom->m_vInfo[INFO_POS] = dynamic_cast<CBullet*>(m_pOwner)->Get_Transform()->m_vInfo[INFO_POS];
	m_pTransformCom->m_vInfo[INFO_POS] += vDir * dynamic_cast<CBullet*>(m_pOwner)->Get_Speed() * 3.f * fTimeDelta;
