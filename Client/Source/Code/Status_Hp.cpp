...
CStatus_Hp::CStatus_Hp(LPDIRECT3DDEVICE9 pGraphicDev)
	: CFrontUi(pGraphicDev), m_bFillCheck(false)
{
}

CStatus_Hp::CStatus_Hp(const CStatus_Hp& rhs)
	: CFrontUi(rhs), m_bFillCheck(false)
{
}
...
{	
	...
	if (pReceiveInfo.Maxhp)
	{
		if (pReceiveInfo.Maxhp < m_tINFO.iNum)
		{
			m_bDead = true;
		}
		else
	...
	if (0 <= pReceiveInfo.Nowhp)
	{
		if (pReceiveInfo.Nowhp < m_tINFO.iNum)
		{
			m_bFillCheck = true;
		}
		else
...
void CStatus_Hp::Set_Receive_PlayerInfo(void)
{
